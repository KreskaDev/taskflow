...
**Feature Branch**: `001-task-capture`
...
- `Space` toggle-done mechanic — canonical scenario US-02.AS-03 owned by slice 004 (daily-planning)
- `E` inline-edit mechanic — canonical scenario US-02.AS-06 owned by slice 004 (daily-planning); superseded by the full editor there
- `Del` delete mechanic — canonical scenario US-08.AS-06 owned by slice 010 (undo); here deletion is permanent (see Constitution Compliance)
...
> Scope note: this slice realizes capture without date parsing. Natural-language date scenarios (US-01.AS-02..AS-05) are delivered in slice 002 (natural-language-dates).
...
> Shortcut coverage: the `Space`, `E`, and `Del` mechanics are implemented in this slice, but their canonical acceptance scenarios are owned by later slices (US-02.AS-03 and US-02.AS-06 → slice 004; US-08.AS-06 → slice 010). See Provenance.
...
> Slice scope for ENT-01: this slice persists `id`, `title`, `status`, `created_at`, `completed_at`, and `updated_at`. The remaining attributes (description, priority, due date, labels, project reference, cycle reference, recurrence rule) are reserved as nullable, forward-compatible columns and are populated by their owning slices (002, 003, 004, 005, 007, 008). Keeping the full status enum from day one avoids a later enum migration.
...
Additionally deferred to later slices within the MVP (not part of slice 001): natural-language dates (002), projects (003), priorities/Today/Upcoming/full editor (004), labels (005), Kanban board (006), cycles (007), recurring tasks (008), command palette & search (009), undo (010), data export/import (011), dark/light theming (012).
