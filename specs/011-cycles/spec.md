...
- Slice 010 (project-board-kanban) — provides the Project List view groupable by cycle (FR-024) and the status-column model whose statuses feed the Cycle view's breakdown
...
This is a Tier A slice — cycles are personal to their owner, so only per-user isolation applies (no project-membership or role surface is introduced here).

- **FR-065**: Every query MUST be scoped to data the caller owns or has membership access to (per-user isolation).

The slice's cycle command and query handlers ENFORCE FR-065 at the handler layer (deny-by-default) — they do not merely reference it. Every cycle create/edit/close/delete, every cycle assignment (`#`), and every metrics or selector read (`G C`) is scoped to the authenticated caller's own data; a request targeting another user's cycle MUST be denied.
...
- **III. Instant Response**: cycle assignment, the metrics render, and rollover confirmation paint an optimistic result within ~16ms of the triggering keypress while the C# API reconciles asynchronously (p95 < 200ms); skeleton screens are permitted for the network-bound metrics and cycle fetch (Principle IV). Real-time reconciliation under last-write-wins (a remote SignalR patch must yield to a pending local optimistic mutation until its server-ack) applies wherever a cycle's tasks live in a shared project; this slice introduces no shared-project surface of its own, so no live fan-out is exercised here, but cycle assignment on a shared-project task reconciles per this rule.
...
- **VIII. Test-First**: each owned acceptance scenario above (US-05.AS-01..AS-07), plus EC-04, EC-10, and EC-12, is independently testable (Red-Green-Refactor). Integration tests cover the authorization of every cycle command/query handler — a request for a cycle the caller does not own MUST be denied.
- **IX. Authentication & Authorization**: every cycle operation is authenticated and authorized deny-by-default at the API/handler layer. Cycles are Tier A (per-user isolation): a caller may only create, read, edit, close, delete, or assign tasks to cycles within their own data, and every cycle query is scoped to the caller's identity (FR-065). The `#` cycle selector lists only the caller's own cycles, the `G C` metrics view aggregates only the caller's tasks, and the rollover/deletion guards run against the caller's own cycles; no cross-user cycle is ever readable or writable. The cycle command and query handlers ENFORCE this isolation (handler-level), not merely reference it.
