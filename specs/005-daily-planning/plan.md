...
This slice realizes the **personal-visibility / ownership** authorization branch in full (FR-065 ownership half, FR-068 deny-by-default) and structures the Today/Upcoming query handlers and the editor command handlers as a **dispatch-by-visibility seam** with the shared-project membership + role arm a **named, not-yet-realized branch**. ⚠ **This is governed by research open-question #1 (the BLOCKER)**: the spec mandates the membership/role branch (FR-066/FR-067) as in-scope with SC-016 deny tests (viewer-mutation-deny, non-member-read-deny), but the `ProjectMembership` substrate does not exist (deferred to slice 007 by slice 004 R11) and slice 005's "Depends on" lists only 003+004 — so that branch and those two deny tests are **not writable against the current substrate**. The plan surfaces this, does **not** silently engineer it away, and does **not** pull `ProjectMembership` forward (that is slice 007's owned scope). See Complexity Tracking.
...
│           └── EditTaskTests.cs                  # (NEW) whole-object replace + omitted-field→422 + project reuse + version_conflict + ALLOW + DENY(404)
│                                                  #   ⚠ SC-016 viewer-deny / non-member-deny: BLOCKED pending slice-007 ProjectMembership (Complexity Tracking)
│
...
### Ownership realized in full; the membership arm is a named seam (R10, governed by the BLOCKER)
Every new handler is deny-by-default, enforced at the handler (FR-068), dispatched by visibility (FR-065). The **ownership** arm is live (scoped reads, owner-coerced writes, foreign id → 404, allow + deny test per handler). The **shared-project membership + role** arm (FR-067) is a named, not-yet-realized branch — ⚠ the `ProjectMembership` substrate is slice 007 (the BLOCKER); the SC-016 viewer-deny / non-member-deny tests are blocked until it exists. The handlers are shaped as a **dispatch-by-visibility seam** so whichever way the blocker resolves, the membership arm fills in without reshaping the query/command.
...
> Of the above, only **open question #1 (the BLOCKER)** is an unresolved gate: it requires a sequencing/spec decision before slice 005 can ship *as specified* (with the shared half). The remaining items are accepted, scoped deferrals — FR-051's no-op is correct (no migration), the plugin/renderer/index are scoped to their owning need, and none is an unjustified constitution violation.
