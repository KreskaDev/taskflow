...
- **Branches realized**: **ownership** (personal/Inbox) AND **shared-project membership + role**. Every handler is **deny-by-default, enforced at the handler** (FR-068).
- **Reads** (`GetTodayTasks`, `GetUpcomingTasks`): scoped `WHERE deleted_at IS NULL AND (created_by = caller OR project_id ∈ ListProjectIdsForUserAsync(caller))` + the §6 view predicates — the caller's own tasks PLUS tasks in shared projects the caller is a current member of; a non-member's tasks are absent (FR-065/FR-066).
- **Writes** (`SetPriority`, `RescheduleDueDate`, `EditTask`, and the now-membership-aware `SetTaskDone`): coerce no owner from the wire and dispatch by the containing project's visibility via `TaskAccessGuards.LoadWritableTaskAsync(EffectiveRole.Editor)` — personal/Inbox → ownership (foreign/absent/tombstoned → **404**); shared → `RequireRole(Editor)` (viewer member → **403**, non-member → **404**). `EditTask`'s project field checks ownership of the **target project** ONLY when it actually changes (an unchanged `projectId` is not a move and skips the check, so an editor editing a shared task they don't own is not spuriously 404'd). `createdBy` is **provenance only**, never a standalone grant (FR-066).
...
- **Dispatch-by-visibility seam**: both arms are **REALIZED** (BLOCKER resolved — slice 007 sequenced first). The **ownership arm** (personal/Inbox) and the **shared-project membership + role arm** (FR-067: viewer=read, editor/owner=write) both fill the seam via the `TaskAccessGuards.LoadWritableTaskAsync` write dispatch and the `ListDueInRangeReadableAsync` read scope (`created_by = caller OR project_id ∈ ListProjectIdsForUserAsync(caller)`) — no query/command reshape was needed, as designed. `SetTaskDone` is now membership-aware too (spec L127 deviation).
- **Test coverage (Constitution VIII + governance gate)**: **every** new/changed handler ships an **allow** and a **deny** integration test through the real DB. Personal deny = another user's task → 404. ✅ The SC-016 **viewer-deny** (→ 403) / **non-member-deny** (→ 404, or absent on the collection reads) tests are **realized and green** (`SetPriorityTests`/`RescheduleDueDateTests`/`EditTaskTests`/`SetTaskDoneSharedAuthzTests`/`GetTodayTasksTests`/`GetUpcomingTasksTests`).
