...
- FR-065 (per-user isolation — see Access control below)
...
This is a Tier A slice — its operations are authorized by per-user isolation alone. The Today and Upcoming query handlers and the priority/edit/reschedule command handlers ENFORCE the following at the handler level (not merely reference it); authorization is deny-by-default.
...
- **IX. Authentication & Authorization**: this slice's query and command handlers are authorized deny-by-default. As a Tier A slice it relies on per-user isolation (FR-065): the Today and Upcoming queries are scoped to the caller's identity, and the priority/edit/reschedule mutations operate only on tasks the caller owns. No unauthenticated request reaches a handler; every read and write is enforced at the API/handler layer.
...
The other assumptions established in slices 002–004 (web platform, Polish date parsing, no subtasks, data format) also continue to apply.
