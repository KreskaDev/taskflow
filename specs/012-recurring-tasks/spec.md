...
- FR-008 (generate next instance on completion on/after due; carry-forward fields including assignees, and resets)
...
This slice is **Tier A** (per-user isolation only): recurrence rules and next-instance generation operate on the caller's own tasks. This slice's command and query handlers ENFORCE FR-065 at the handler level — the on-completion next-instance generation, the on/after-due deferred-spawn check, and any read of recurring tasks are all scoped to the caller's identity. FR-065 is not merely referenced; it is enforced by the handlers themselves, deny-by-default.
...
- **EC-05 — Recurring task early completion**: If a recurring task is completed before its due date, no new instance is spawned immediately. The system generates the deferred instance on the next application startup on or after the due date, or during periodic checks while the app is running. The deferred instance keeps the same assignees as the completed instance.
...
- **V. Connected, Server-Authoritative**: recurrence computation and next-instance generation are performed and persisted through the application's own C# API into PostgreSQL, which is the source of truth; the Next.js client surfaces the successor via optimistic UI and reconciles against the server. The on/after-due check (at startup and periodically) drives generation through that same API. The only data dependency is the application's own API and database — no third-party runtime data services (SC-004, owned by slice 002, task-capture); the sole external runtime dependency is Google OAuth for authentication, never for application data.
...
- **VII. Data Integrity & Resilience**: FR-008's deterministic carry-forward (including assignees) and resets prevent silent data loss across instance generation; FR-049 surfaces any generation error with a recovery suggestion and FR-050 logs it with structured context; FR-051 keeps the backup hook in place ahead of the schema change that adds the recurrence rule.
- **VIII. Test-First**: each owned acceptance scenario above, plus EC-05, is independently testable (Red-Green-Refactor), including authorization tests that a recurrence operation on another user's task is denied.
- **IX. Authentication & Authorization**: authorization is deny-by-default and enforced at the handler layer for every read and write in this slice. As a Tier A slice, this slice enforces **per-user isolation** (FR-065): generating the next instance on completion, running the on/after-due deferred-spawn check, and reading recurring tasks are all scoped to the caller's identity — a caller may only drive recurrence on tasks they own. When a recurring task lives in a shared project, the carried-forward assignees are members of that project; the handler still authorizes the operation against the caller's identity and the project's membership before generating the successor.
