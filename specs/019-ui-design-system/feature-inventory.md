...
| INV-031 | Given Inbox tasks, When the list renders, Then it is a labelled `role="grid"` of `role="row"` rows (single `display:contents` gridcell each — post-010 nested-interactive remediation), each showing: status indicator, title, priority badge (when set, text `P0`–`P3`), label chips (name text), due date; done rows are visually distinguished (strikethrough/`data-status`) beyond color alone. | US-18 AS-02 (pre-form), FR-044 | [E]+[C] |
| INV-032 | Given the Inbox task grid focused, When the user presses ↑/↓, Then selection moves with `aria-selected` and the grid's `aria-activedescendant` tracking it; default selection is the first row. (Composite-widget operability — RETAINED after FR-111 as roving selection inside the widget.) | FR-042/043, UIT-090..093 | [E] |
...
| INV-041 | Given ~60+ tasks (virtualized list), When the user scrolls past the selected row, Then the selected row stays mounted and addressable (`aria-activedescendant` never dangles) and selection keys still work. | Constitution Performance | [E] |
...
| INV-125 | Given editors after the redesign, When the user presses Ctrl+Enter / Esc inside them, Then save/cancel still work (FR-030 editing keys are NOT shortcuts and survive), and composite-widget keys (↑/↓ selection, Space toggle, Enter open) still work INSIDE the focused task grid. | FR-030, FR-042..047 | [E] |
...
| INV-142 | Given a project with tasks in several statuses, When the Board renders, Then EXACTLY four columns appear in order Backlog, Do zrobienia, W toku, Zrobione — each a labelled `role="list"` (cards `role="listitem"` — PR #8 remediation) with `aria-label` „<label>, N zadań" and a visible live count; cards show sanitized title, due/priority/label chips and assignee avatars on catalog components; an empty column renders the catalog EmptyState. | US-03 AS-03, FR-025, FR-110 | [E]+[A] |
