...
role) and `last_owner` (409, last-owner guard) were pre-provisioned in slice 004 and are **first used** here;
...
leave/be-removed/be-demoted (409 `last_owner`); unshare → personal again, all members lose access, tasks preserved;
...
- [ ] T023 [P] [US-12] **(write first — RED)** Create `apps/api/tests/TaskFlow.IntegrationTests/TaskManagement/ChangeMemberRoleTests.cs` — `editor↔viewer` toggle; re-sending the current role is a **no-op + version bump**, not an error (R5); a **demotion (editor→viewer) raises `MembershipRevoked`** (a promotion viewer→editor raises none, R5); **target == owner → 409 `last_owner`** (checked before the row lookup, R7); target who is neither owner nor member → **404**; **ALLOW** + **DENY** (member caller → 403, non-member → 404); stale `version` → **409** (covers T024).
- [ ] T024 [US-12] Create `apps/api/src/TaskFlow.Application/TaskManagement/ChangeMemberRole.cs` (+ `ChangeMemberRoleRequest.cs` + validator) — owner-only; `EnsureNotLastOwner` precedes the row lookup; set member `role`; **raises `MembershipRevoked` on a demotion (editor→viewer)** (no event on a promotion, R5); `version` — **and** `MemberEndpoints.ChangeRole` (`PATCH /api/projects/{id}/members/{userId}`) (depends on T007, T009, T015; RED via T023).
- [ ] T025 [P] [US-12] **(write first — RED)** Create `apps/api/tests/TaskFlow.IntegrationTests/TaskManagement/RemoveMemberTests.cs` — remove a member → the **removed user loses ALL access** (their next read of project/tasks/members → **404**, R10) + raises `MembershipRevoked`; **target == owner → 409 `last_owner`** (before the row lookup, R7); target neither owner nor member → **404**; **ALLOW** + **DENY** (member caller → 403, non-member → 404); stale `version` → **409** (covers T026).
...
- [ ] T027 [P] [US-12] **(write first — RED)** Create `apps/api/tests/TaskFlow.IntegrationTests/TaskManagement/LeaveProjectTests.cs` — a non-owner member leaves → loses ALL access (next read → **404**, R10) + raises `MembershipRevoked`; **caller == owner → 409 `last_owner`** (before the row lookup — the owner has no row, R7); non-member caller → **404**; **ALLOW** (member self) + **DENY** (non-member → 404); stale `version` → **409** (covers T028).
...
- [ ] T047 [US-12] **(write first — RED, then GREEN)** Create `apps/web/tests/e2e/sharing.spec.ts` (Playwright) — **AS-01..AS-06** + invite-unknown-email (422) + viewer-read-only (**verifies UI gating via `ProjectResponse.role`, NOT server write-denial** — server-side viewer-write enforcement lands in slice 008) + last-owner-guard (409) + removed-member-loses-access (404) + the `personal↔shared` reversibility round-trip — all **live** against the wired DOM (no `test.fixme`) (depends on T046).
...
- **Deny-shape rule** (R9): **non-member → 404 `not_found`** (existence not disclosed across the membership boundary); **insufficient-role member → 403 `forbidden`** (the member knows it exists); **last-owner-targeted → 409 `last_owner`** (recoverable — "transfer first", checked **before** the membership-row lookup, R7).
