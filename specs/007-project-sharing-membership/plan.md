...
│   │   │       ├── ChangeMemberRole.cs            # (NEW) owner-only; set member role editor↔viewer; target==owner → last_owner; version (R5,R7)
...
│           └── RoleOperationDenyMatrixTests.cs    # (NEW) THE SC-016 matrix: viewer-denied-write, editor-denied-manage(incl. delete), non-member-denied-read(404), removed-member(404), last-owner(422)
│
...
All findings are planning-doc refinements (no code exists yet); none blocks the slice's design, which the reviewer otherwise found sound (coverage strong, authorization model sound, no privilege-escalation hole, FR-051 correctly LIVE). The two policy decisions (H1 event, M1 status) are now fixed; M2/M3/L1 are carried into the T-phase via this table.
