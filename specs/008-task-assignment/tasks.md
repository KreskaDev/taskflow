...
- [x] T018 [US1] Clear assignments on membership loss (R5). **EVENT-DRIVEN** (revised from inline — the slice-007 events were built for this): turn the no-op `ProjectUnsharedHandler` + `MembershipRevokedHandler` in `MembershipEventHandlers.cs` into real consumers — `ProjectUnshared` → `ClearAssigneesForProjectAsync`; `MembershipRevoked` → `ClearAssigneesForUserInProjectAsync` **guarded by a current-membership re-check** (a demotion keeps assignments). **INLINE** for `DeleteProject.cs` `move_to_inbox` (the bulk move nulls `project_id` first) → `ClearAssigneesForProjectAsync` BEFORE the move. `UnshareProject`/`RemoveMember`/`LeaveProject` are UNCHANGED (they already raise the events). Depends T009; RED via T017.
- [x] T019 [US1] Confirm `EditTask` AND `MoveTaskToProject` clear assignees on a real project change — `Task.MoveToProject` AND `Task.EditTask` each carry the clear (T004; `EditTask` does NOT route through `MoveToProject`, so it has its own guard). Asserted by domain tests (`EditTask`-clears) + the `AssignmentCleanupTests` `/edit`-to-inbox integration test. RED via T003/T017.
