...
Implemented via new `ITaskRepository` set-based deletes (`ClearAssigneesForProjectAsync(projectId)` and `ClearAssigneesForUserInProjectAsync(projectId, userId)`) invoked inside the existing slice-007 handlers (transactional, alongside the membership mutation). These are **changed data handlers** → they get assignment-cleanup tests (the membership allow/deny is inherited from slice 007; the new assertion is "assignment cleared").
...
**Rationale**: the spec is explicit that assignment must not survive membership loss. Clearing inside the slice-007 transaction keeps it atomic with the membership change (one-aggregate caveat: assignee rows belong to the Task aggregate, but a set-based `ExecuteDelete` over the project's tasks is a bulk maintenance operation, like the slice-004 `MoveProjectTasksToInboxAsync`/`OrphanChildrenAsync` bulk ops, run in the same per-message transaction). Account deletion is handled for free by the FK cascade.
...
- **(b) Raise events and clear asynchronously** — rejected; the clear must be atomic with the membership change (no window where access is revoked but the assignment lingers).
