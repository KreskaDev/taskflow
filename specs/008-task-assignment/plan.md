...
│   │   │   ├── MembershipEventHandlers.cs        # (MODIFY) ProjectUnshared/MembershipRevoked no-ops → real assignment-clear consumers, demotion-guarded (R5). UnshareProject/RemoveMember/LeaveProject stay UNCHANGED (already raise the events)
│   │   │   ├── DeleteProject.cs                 # (MODIFY) move_to_inbox branch ClearAssigneesForProjectAsync BEFORE the bulk move (bypasses the aggregate, R5)
│   │   │   ├── Task.cs (Domain)                   # (MODIFY) MoveToProject AND EditTask clear assignees on a real project change (R5; EditTask doesn't route through MoveToProject)
...
| **Cross-slice touch: assignment cleanup on membership loss** | **Required** by FR-066/spec scenario 7 — assignment must not outlive membership. | EVENT-DRIVEN (R5): the slice-007 `ProjectUnshared`/`MembershipRevoked` no-op handlers (built naming slice 008 as consumer) become real assignment-clear consumers, demotion-guarded; the membership-removal handlers themselves are UNCHANGED. `DeleteProject` move_to_inbox clears inline (bulk move bypasses the aggregate). Safe async because assignee is provenance-only (no access window). Non-author authorization review (Principle IX). |
