...
/**
 * The groupable project List (slice 010, T015 — FR-024, D9, US-03.AS-07). Pins the
 * DailyView-proven grouped-listbox contract on the new component: ONE `role="listbox"`,
 * `role="group"` + `aria-label` per group, a FLAT selection index across groups — plus the
 * visible group-by control („Grupuj: Brak | Status | Priorytet”; „wg cyklu” arrives with
 * slice 011 and MUST NOT render here).
 */
...
describe("GroupedTaskList — grouped listbox with a flat index (FR-024, D9) [INV-146]", () => {
  it("renders ONE listbox containing a role=group with an accessible label per group", () => {
    renderGrouped();

    expect(screen.getAllByRole("listbox")).toHaveLength(1);
    expect(screen.getByRole("group", { name: "Do zrobienia" })).toBeTruthy();
    expect(screen.getByRole("group", { name: "Zrobione" })).toBeTruthy();
    expect(screen.getByRole("group", { name: "Anulowane" })).toBeTruthy();
  });

  it("renders cancelled tasks inside the „Anulowane” group (visible HERE, hidden on the Board)", () => {
    renderGrouped();

    const cancelled = screen.getByRole("group", { name: "Anulowane" });
  ...
  it("moves selection ACROSS group boundaries with one flat arrow-key index", () => {
    ...
    expect(listbox.getAttribute("aria-activedescendant")).toBe(taskOptionId("t-todo"));

    fireEvent.keyDown(listbox, { key: "ArrowDown" });
    expect(listbox.getAttribute("aria-activedescendant")).toBe(taskOptionId("t-done"));

    fireEvent.keyDown(listbox, { key: "ArrowDown" });
    expect(listbox.getAttribute("aria-activedescendant")).toBe(taskOptionId("t-cancelled"));
  });
});
